...
CDdeltae = 2.*ap.K.*(CW0- ap.CLmin).*ap.CLde;
CXdeltae = -CDdeltae.*cos(alpha0) + ap.CLde.*sin(alpha0);
Cmdeltae = ap.Cmde;
CZdeltae = -CDdeltae.*sin(alpha0) - ap.CLde.*cos(alpha0);
CZdeltat = 0;
CXdeltat = 2.*((ap.Sprop.*ap.Cprop)./(ap.S.*u0.^2)).*(ap.kmotor - u0).*(u0 + 2.*dt0.*(ap.kmotor- u0));
Cmdeltat = 0;
Xdeltae = 0.5.*rho.*u0.^2.*ap.S.*CXdeltae;
Mdeltae = 0.5.*rho.*u0.^2.*ap.S.*ap.c.*Cmdeltae;
Zdeltae = 0.5.*rho.*u0.^2.*ap.S.*CZdeltae;
Zdeltat = 0.5.*rho.*u0.^2.*ap.S.*CZdeltat;
Xdeltat = 0.5.*rho.*u0.^2.*ap.S.*CXdeltat;
Mdeltat = 0.5.*rho.*u0.^2.*ap.S.*Cmdeltat;
% Matrices
Alon1 = [  Xu/ap.m,           Xw/ap.m,               0,                          -ap.g*cos(theta0);
       ...
       0,              0,                  1,                             0];
Blon = [Xdeltae./ap.m, Xdeltat;...
       Zdeltae./(ap.m-Zwdot), Zdeltat./(ap.m-Zwdot);...
       (Mdeltae./(ap.Iy))+(Mwdot./ap.Iy).*(Zdeltae./(ap.m-Zwdot)), (Mdeltat./ap.Iy)+(Mwdot./ap.Iy).*(Zdeltat./(ap.m-Zwdot));...
       0, 0;...
       0, 0;...
       0, 0];
...
Cldeltar = ap.Cldr;
Cndeltar = ap.Cndr;
Ldeltaa = 0.5.*rho.*u0.^2.*ap.S.*ap.b.*Cldeltaa;
Ndeltaa = 0.5.*rho.*u0.^2.*ap.S.*ap.b.*Cndeltaa;
Ldeltar = 0.5.*rho.*u0.^2.*ap.S.*ap.b.*Cldeltar;
Ndeltar = 0.5.*rho.*u0.^2.*ap.S.*ap.b.*Cndeltar;
Ydeltar = 0.5.*rho.*u0.^2.*ap.S.*ap.CYdr;
Alat1 = [Yv/ap.m, Yp/ap.m, (Yr/ap.m - u0), ap.g*cos(theta0);
   ...
   0, 1, tan(theta0), 0];
Blat = [0, Ydeltar;...
       G3.*Ldeltaa + G4.*Ndeltaa, G3.*Ldeltar + G4.*Ndeltar;...
       G4.*Ldeltaa + G8.*Ndeltaa, G4.*Ldeltar + G8.*Ndeltar;...
       0, 0,;...
       0, 0;...
       0, 0];
